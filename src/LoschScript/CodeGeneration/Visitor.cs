...
internal class Visitor : LoschScriptParserBaseVisitor<Type>
{
    ...
    {
        TypeBuilder tb = Context.Types.Select(t => t.Builder).First(t => t.FullName == $"{(string.IsNullOrEmpty(CurrentFile.ExportedNamespace) ? "" : $"{CurrentFile.ExportedNamespace}.")}{context.Identifier().GetText()}");
