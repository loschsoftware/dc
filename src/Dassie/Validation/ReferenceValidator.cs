    ...
    public static Reference[] ValidateReferences(Reference[] references)
    {
        ...
        foreach (Reference reference in references)
        {
            if (reference is AssemblyReference ar && !File.Exists(Path.GetFullPath(Path.Combine(GlobalConfig.RelativePathResolverDirectory, ar.AssemblyPath))))
                EmitErrorMessage(0, 0, 0, DS0023_InvalidAssemblyReference, $"The referenced assembly '{(reference as AssemblyReference).AssemblyPath} does not exist.", ProjectConfigurationFileName);
            else
                valid.Add(reference);
            ...
            if (reference is FileReference fr && !File.Exists(Path.GetFullPath(Path.Combine(GlobalConfig.RelativePathResolverDirectory, fr.FileName))))
                EmitErrorMessage(0, 0, 0, DS0024_InvalidFileReference, $"The referenced file '{(reference as FileReference).FileName} does not exist.", ProjectConfigurationFileName);
            else
                valid.Add(reference);
        }
