...
--feature:System.Threading.ThreadPool.UseWindowsThreadPool=true");
    ...
    private void AddLinkerDefaultArguments(out string linkerPath)
    {
        ...
        // TODO: Try to implement a more robust version of this
        string kitPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ProgramFilesX86), "Windows Kits", "10", "Lib");

        if (os == "win" && (!Directory.Exists(libPath) || !Directory.Exists(netLibPath) || !Directory.Exists(kitPath)))
        {
            EmitErrorMessage(
                0, 0, 0,
        ...
        kitPath = Path.Combine(Directory.GetDirectories(kitPath).Where(k => k.Split(Path.DirectorySeparatorChar)[^1].StartsWith("10.")).Last());
...
/LIBPATH:""{Path.Combine(kitPath, "um", platform)}""
/LIBPATH:""{Path.Combine(kitPath, "ucrt", platform)}""
