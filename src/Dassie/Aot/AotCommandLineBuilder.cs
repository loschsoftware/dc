...
internal class AotCommandLineBuilder
{
    ...
    public string GenerateIlcArgumentList()
    {
        ...
        return _arglistBuilder.ToString();
    }
    ...
    private void AddDefaultReferences()
    {
        ...
        string sdkDir = Path.Combine(_config.RuntimePackageRootDirectory, "runtimes", _config.Config.RuntimeIdentifier, "native");
        IEnumerable<string> sdkFiles = ["System.Private.CoreLib.dll", "System.Private.Reflection.Execution.dll", "System.Private.StackTraceMetadata.dll", "System.Private.TypeLoader.dll"];
        sdkFiles = sdkFiles.Select(f => $"-r:\"{Path.Combine(sdkDir, f)}\"");

        string frameworkDir = Path.Combine(_config.RuntimePackageRootDirectory, "runtimes", _config.Config.RuntimeIdentifier, "lib", $"net{target}");
...
--feature:System.Threading.ThreadPool.UseWindowsThreadPool=true");
    ...
    private void AddLinkerDefaultArguments(out string linkerPath)
    {
        ...
        string libPath = Directory.GetDirectories(Path.Combine(sdkPathV10, "lib")).Where(k => k.Split(Path.DirectorySeparatorChar)[^1].StartsWith("10.")).Last();
        ...
        if (os != "win")
        {
            EmitErrorMessage(
                ...
                $"Ahead-of-time compilation is not supported for systems other than Windows yet. To finish the compilation, enable the 'KeepIntermediateFiles' setting and link the generated object files manually.");

            linkerPath = "";
            return;
        }

        if (!Directory.Exists(libPath) || !Directory.Exists(netLibPath))
        {
            EmitErrorMessage(
                0, 0, 0,
                DS0070_WinSdkToolNotFound,
                "The Windows SDK or parts of it could not be found on this machine, which is required for linking object files generated by the AOT compiler on Windows systems. To finish the compilation, either install the Windows SDK or enable the 'KeepIntermediateFiles' setting to prevent the generated object files from being deleted and link them manually.",
                CompilerExecutableName);
        }

        string msvcRootPath = (string)MsvcRootPathProperty.Instance.GetValue() ?? "";
        if (!Directory.Exists(msvcRootPath))
        {
            EmitErrorMessage(
                0, 0, 0,
                DS0261_ExtensionsLocationPropertyInvalidPath,
                $"To enable AOT compilation, set the 'Locations.MsvcRootPath' global property to the root directory of the current installation of MSVC. This needs to be done only once.");
...
""{Path.Combine(_config.RuntimePackageRootDirectory, "runtimes", _config.Config.RuntimeIdentifier, "native", "bootstrapper.obj")}""
""{Path.Combine(_config.RuntimePackageRootDirectory, "runtimes", _config.Config.RuntimeIdentifier, "native", "aotminipal.lib")}""
""{Path.Combine(_config.RuntimePackageRootDirectory, "runtimes", _config.Config.RuntimeIdentifier, "native", "Runtime.WorkstationGC.lib")}""
""{Path.Combine(_config.RuntimePackageRootDirectory, "runtimes", _config.Config.RuntimeIdentifier, "native", "eventpipe-disabled.lib")}""
""{Path.Combine(_config.RuntimePackageRootDirectory, "runtimes", _config.Config.RuntimeIdentifier, "native", "Runtime.VxsortEnabled.lib")}""
""{Path.Combine(_config.RuntimePackageRootDirectory, "runtimes", _config.Config.RuntimeIdentifier, "native", "standalonegc-disabled.lib")}""
""{Path.Combine(_config.RuntimePackageRootDirectory, "runtimes", _config.Config.RuntimeIdentifier, "native", "System.Globalization.Native.Aot.lib")}""
""{Path.Combine(_config.RuntimePackageRootDirectory, "runtimes", _config.Config.RuntimeIdentifier, "native", "System.IO.Compression.Native.Aot.lib")}""
""advapi32.lib""
""bcrypt.lib""
""crypt32.lib""
""iphlpapi.lib""
