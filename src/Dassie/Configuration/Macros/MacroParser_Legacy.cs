...
internal partial class MacroParser_Legacy
{
    public MacroParser_Legacy(bool addDefaults = true)
    {
        if (addDefaults)
            AddDefaultMacros();
    }
    ...
    public static List<(string Key, bool IsPredefined, string Value)> GetMacrosForProject(DassieConfig config, bool addBuiltIn = true)
    {
        ...
        MacroParser_Legacy mp = new(addBuiltIn);
...
#else
        string compilerDir = Path.GetDirectoryName(typeof(MacroParser_Legacy).Assembly.Location) + Path.DirectorySeparatorChar;
        string compilerPath = typeof(MacroParser_Legacy).Assembly.Location;
#endif
